...
namespace imbNLP.Data.extended.wordnet
{
    ...
    public class languageManagerWordnet : tokenQueryResolverBase
    {
        ...
        public wordnetSymsetResults query_eng_symset(List<String> symsetCodes, ILogBuilder response)
        {
            ...
            if (response != null) response.log("wordnet_eng_symset(" + symsetCodes.Join(',') + ") = rows[" + matches.Count + "] => words[" + output.GetEnglish().Count + "]");
        ...
        public wordnetSymsetResults query_eng(List<String> eng_tokens, ILogBuilder response)
        {
           ...
           if (response != null) response.log("wordnet_eng(" + eng_tokens.Join(',') + ") = rows[" + matches.Count() + "]");
            ...
            if (response != null) response.log("wordnet_eng(" + eng_tokens.Join(',') + ") = rows[" + matches.Count() + "] => token-symset[" + output.Count +"]" );
        ...
        public wordnetSymsetResults query_srb_symset(List<String> symsetCodes, ILogBuilder response)
        {
            ...
            if (response != null) response.log("wordnet_srb_symset(" + symsetCodes.Join(',') + ") = rows[" + matches.Count + "] => words[" + output.GetEnglish().Count + "]");

    ...
    public class termExploreModel: termExploreItem
    {
       ...
       // [XmlElement(typeof(Int32), "new_links_c2c")]
        ...
        public void ToString(ILogBuilder loger, bool expanded=false, bool showInstances=true)
        {
            ...
            if (synonyms.Any()) loger.AppendLine("Related terms: " + synonyms.Join(','));
            if (wordnetSynonyms.Any()) loger.AppendLine("Not accepted: " + wordnetSynonyms.Join(','));


            if (translations.Any()) loger.AppendLine("Translations: " + translations.Join(','));
            if (translationRelated.Any()) loger.AppendLine("Not accepted: " + translationRelated.Join(','));

           
           // if (wordnetSynonymSerbian.Any()) loger.AppendLine("WordNet translated: " + wordnetSynonymSerbian.Join(','));
            if (wordnetPrimarySymsets.Any()) loger.AppendLine("Symsets: " + wordnetPrimarySymsets.Join(','));
            if (wordnetSecondarySymsets.Any()) loger.AppendLine("Not accepted: " + wordnetSecondarySymsets.Join(','));

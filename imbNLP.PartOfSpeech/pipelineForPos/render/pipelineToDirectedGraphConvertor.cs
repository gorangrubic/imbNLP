...
namespace imbNLP.PartOfSpeech.pipelineForPos.render
{
    ...
    public class pipelineToDirectedGraphConvertor
    {
        ...
        Dictionary<IPipelineNode, Node> nodeRegistry = new Dictionary<IPipelineNode, Node>();
        aceDictionarySet<IPipelineNode, IPipelineNode> ordinalSet = new aceDictionarySet<IPipelineNode, IPipelineNode>();
        ...
        public DirectedGraph GetModelGraph(pipelineModel<pipelineTaskSubjectContentToken> model, Color[] colors=null, Int32 limit = 10)
        {
            ...
            st_basic.Stroke = colors[0].toHexColor();
            st_basic.StrokeThinkness = 2;
            st_basic.Background = colors[1].toHexColor();

            st_bin.StrokeDashArray = "5,2,5,2";

            st_taskBuilder.Background = colors[2].toHexColor();
            st_transformer.Background = colors[3].toHexColor();

            st_distributor.Background = colors[4].toHexColor();

            st_bin.Background = colors[5].toHexColor();

            st_model.Background = colors[6].toHexColor();
            ...
            while (nextSet.Any())
            {
                ...
                foreach (IPipelineNode parent in nextSet)
                {
                    ...
                    if (parent.nodeType == pipelineNodeTypeEnum.distributor)
                    {
                        ...
                        tl.Stroke = Color.Gray.toHexColor();
                    ...
                    if (parent.GetType().Name.Contains("TaskBuilder"))
                    {
                        if (modelMainNode != null)
                        {
                            ...
                            ntl.Stroke = Color.MediumVioletRed.toHexColor();
                    ...
                    if (parent.parent != null)
                    {
                        ...
                        {
                            l.Stroke = Color.SteelBlue.toHexColor();
                            l.Label = "No";
                            l.StrokeThinkness = 4;
                            output.Links.Add(l);
                        }
                        ...
                        {
                            l.Stroke = Color.DarkGray.toHexColor();
                            l.Label = "Owner";
            ...
            foreach (var pair in ordinalSet)
            {
                ...
                foreach (IPipelineNode p in pair.Value)
                {
                    ...
                    else
                    {
                        ...
                        //ld.Stroke = Color.SteelBlue.toHexColor();
